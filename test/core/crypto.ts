...
import * as acmeCore from "@peculiar/acme-core";
...
context("crypto", () => {
  ...
  acmeCore.cryptoProvider.set(crypto);

  context("Name", () => {

    function assertName(name: asn1X509.Name, text: string) {
      // serialize
      const value = new acmeCore.Name(name).toString();
      assert.strictEqual(value, text);

      // parse
      const name2 = new acmeCore.Name(text);
    ...
    context("Escaped chars", () => {
      ...
      it("parse quoted value", () => {
        ...
        const name = new acmeCore.Name(text);
    ...
    it("json", () => {
      ...
      const name2 = new acmeCore.Name(json);
      assert.strictEqual(name2.toString(), text);
  ...
  context("Pkcs10CertificateRequest", () => {
    ...
    it("read", () => {
      ...
      const csr = new acmeCore.Pkcs10CertificateRequest(Convert.FromBase64(pem));
  ...
  context("x509", () => {
    ...
    it("read", () => {
      const cert = new acmeCore.X509Certificate(Convert.FromBase64(pem));
  ...
  context("X509 certificate generator", () => {
    ...
    it("generate self-signed certificate", async () => {
      ...
      const cert = await acmeCore.X509CertificateGenerator.createSelfSigned({
        serialNumber: "01",
        name: "CN=Test",
        ...
        keys: keys,
        extensions: [
          new acmeCore.BasicConstraintsExtension(true, 2, true),
          new acmeCore.ExtendedKeyUsageExtension(["1.2.3.4.5.6.7", "2.3.4.5.6.7.8"], true),
          new acmeCore.KeyUsagesExtension(acmeCore.KeyUsageFlags.keyCertSign | acmeCore.KeyUsageFlags.cRLSign, true),
          await acmeCore.SubjectKeyIdentifierExtension.create(keys.publicKey),
        ]
      });
    ...
    it("generate ca and user certificate", async () => {
      ...
      const caCert = await acmeCore.X509CertificateGenerator.create({
        serialNumber: "01",
        subject: "CN=Test CA",
        issuer: "CN=Test CA",
        notBefore: new Date("2020/01/01"),
      ...
      const userCert = await acmeCore.X509CertificateGenerator.create({
        serialNumber: "01",
        subject: "CN=Test",
  ...
  context("X509Certificates", () => {
    ...
    const certs: acmeCore.X509Certificates = new acmeCore.X509Certificates();

    before(async () => {
      const alg: EcKeyGenParams & EcdsaParams = { name: "ECDSA", namedCurve: "P-256", hash: "SHA-256" };
      const keys = await crypto.subtle.generateKey(alg, false, ["sign", "verify"]) as CryptoKeyPair;
      certs.push(await acmeCore.X509CertificateGenerator.createSelfSigned({
        name: "CN=Test #1",
        notBefore: new Date("2020/01/01"),
    ...
    it("import/export", async () => {
      ...
      const certs2 = new acmeCore.X509Certificates(raw);
  ...
  context("chain", () => {
    ...
    async function createCert(name: string, cert?: acmeCore.X509Certificate, useKeyId = false) {
      const keys = await crypto.subtle.generateKey(alg, false, ["sign", "verify"]) as CryptoKeyPair;

      const extensions: acmeCore.Extension[] = [];
      const skiExt = await acmeCore.SubjectKeyIdentifierExtension.create(keys.publicKey);
      extensions.push(skiExt);
      if (useKeyId) {
        const akiExt = await acmeCore.AuthorityKeyIdentifierExtension.create(cert ? await cert.publicKey.export() : keys.publicKey);
        extensions.push(akiExt);
      }

      const res = await acmeCore.X509CertificateGenerator.create({
        serialNumber: "01",
        subject: `CN=${name}`,
        ...
        signingKey: cert ? cert.privateKey! : keys.privateKey,
        extensions,
      });
    ...
    const certs = new acmeCore.X509Certificates();
    ...
    it("without key identifiers", async () => {
      const chain = new acmeCore.X509ChainBuilder({
        certificates: certs,
      });
